...
# for soft structure
# L = 120ft = 36.576m; lambda = 25ft/48in = 6.25 , abutment_gap = 1.2in = 30.48mm # new bridge, AL2 no converge
...
# 80% shear force based pushover
# Dy = 2.5370125e+00*25.4/1000 # new soft bridge
# Hc = 25*12*25.4/1000 # soft bridge
# Dy = Dy/Hc*100 # yielding drift
# Fy = 2.0381000e+02*4.448/1000 #soft bridge

# 75% shear force based pushover
Dy = 2.4037267e+00*25.4/1000 # new soft bridge
Hc = 25*12*25.4/1000 # soft bridge
Dy = Dy/Hc*100 # yielding drift
Fy = 2.0381000e+02*4.448/1000 #soft bridge
...
StructureInfo[:,0] = 36.576
StructureInfo[:,1] = 6.25
StructureInfo[:,2] = 30.48
...
for ii in np.random.choice(N,5,False):
    compare_plot(ii,path="02_soft_plot\\")
...
for ii in np.arange(4):
    ...
    # plt.legend()
    plt.tight_layout()
    plt.savefig("02_soft_plot\\"+xlabel[ii][1:-1]+".svg",dpi=100)
    plt.show()
...
def fragility(Sa,true_EDP,sample_EDP,name,EDP_threshold,EDP_std,dr=None,ref_EDP=None,hazard=None):
    ...
    plt.tight_layout()
    plt.savefig("02_soft_plot\\"+name+"_frag75.svg")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("02_soft_plot\\"+name+"_damage75.svg")
    plt.show()
    ...
    plt.savefig("02_soft_plot\\"+name+"_risk75.svg")
    plt.show()

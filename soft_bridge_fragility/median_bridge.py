...
# 80% shear force
# Dy = 2.4457351*25.4/1000 # AL2
# Hc = 22.382*12*25.4/1000
# Dy = Dy/Hc*100 # yielding drift percentage
# Fy = 2.9724300e+02*4.448/1000 #AL2, median bridge

# 75% shear force
Dy = 2.3474020e+00*25.4/1000 # AL2
Hc = 22.382*12*25.4/1000
Dy = Dy/Hc*100 # yielding drift percentage
Fy = 2.9724300e+02*4.448/1000 #AL2, median bridge
...
StructureInfo[:,0] = 27.734
StructureInfo[:,1] = 5.596
StructureInfo[:,2] = 50.0126
...
for ii in np.random.choice(N,5,False):
    compare_plot(ii,path="02_median_plot\\")
...
for ii in np.arange(4):
    ...
    # plt.legend()
    plt.tight_layout()
    plt.savefig("02_median_plot\\"+xlabel[ii][1:-1]+".svg",dpi=100)
    plt.show()
...
for ii in [212,130,630]:
    ...
    # plt.ylim(1e-2,)
    plt.xlim([0,40])
    plt.savefig("02_median_plot\\\Ec_"+str(ii+1)+".svg", dpi=100, transparent=True, bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('$\Delta_c $ (%)')
    plt.ylabel('$F_c$ (MN)')
    plt.savefig("02_median_plot\\Hystersis_"+str(ii+1)+".svg", dpi=100, transparent=True, bbox_inches='tight')
    plt.show()
    
    # compare_plot(ii,path="02_median_plot\\resp_")
...
for ii in np.arange(len(xlabel)):
    ...
    # plt.legend()
    plt.tight_layout()
    # plt.savefig("02_median_plot\\"+xlabel[ii][1:4]+".svg",dpi=100)
    plt.show()
# In[sampling]
Pc,Pb,Ec = [],[],[]
...
def fragility(Sa,true_EDP,sample_EDP,name,EDP_threshold,EDP_std,dr=None,ref_EDP=None,hazard=None):
    ...
    plt.tight_layout()
    plt.savefig("02_median_plot\\"+name+"_frag75.svg")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("02_median_plot\\"+name+"_damage75.svg")
    plt.show()
    ...
    plt.savefig("02_median_plot\\"+name+"_risk75.svg")
    plt.show()
